...
const navItems = [
  ...
  { href: '/student/apply', label: 'Apply for Course', icon: FileText },
  { href: '/student/documents', label: 'Documents', icon: Upload },
  { href: '/student/chat', label: 'AI Assistant', icon: MessageSquare },
  { href: '/student/book-counseling', label: 'Book Counseling', icon: CalendarDays },
]
    ...
    <aside className="w-64 h-screen bg-slate-900 border-r border-slate-700/50 flex flex-col fixed left-0 top-0 z-30">
      ...
      <div className="p-6 border-b border-slate-700/50">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-violet-400 text-xs">Admission Portal</p>
          </div>
        </div>
      </div>
          ...
          return (
            <Link
              ...
              className={cn(
                ...
                isActive
                  ? 'bg-gradient-to-r from-violet-600/20 to-indigo-600/20 text-violet-300 border border-violet-500/30'
                  : 'text-slate-400 hover:text-white hover:bg-slate-800'
              )}
            >
              <Icon className={cn('h-4 w-4 shrink-0', isActive ? 'text-violet-400' : 'text-slate-500 group-hover:text-slate-300')} />
      ...
      <div className="p-4 border-t border-slate-700/50">
        <div className="flex items-center gap-3 px-3 py-2 mb-2">
          <div className="w-8 h-8 bg-gradient-to-br from-violet-500 to-indigo-500 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0">
